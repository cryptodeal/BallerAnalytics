...
import type { DefaultLoadingManager } from 'three';
import type { SVGLoaderSyntaxError } from './types';

class SVGLoader extends Loader {
	public defaultDPI: number;
	public defaultUnit: string;
	constructor(manager?: typeof DefaultLoadingManager) {
		super(manager);
	...
	parse(text) {
		...
		function parsePathNode(node) {
			...
			const commands: string[] = d.match(/[a-df-z][^a-df-z]*/gi);

			for (let i = 0, l = commands.length; i < l; i++) {
				const command = commands[i];

				const type = command.charAt(0);
				const data = command.substring(1).trim();
				...
				for (let tIndex = transformsTexts.length - 1; tIndex >= 0; tIndex--) {
					...
					if (openParPos > 0 && openParPos < closeParPos) {
						...
						const array = parseFloats(
							transformText.substring(openParPos + 1, closeParPos - openParPos - 1)
						);
