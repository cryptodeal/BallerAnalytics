<script lang="ts">
	...
	let svg,
		...
		height = 1,
		simulation,
		transform = zoomIdentity.translate(0, 0).scale(4),
		cxnData: CXNData[] = [],
		nodeData: NodeData[] = [];

	const r = 5;

	$: if (nodes && cxns) startSimulation();
	...
	onMount(() =>
		select(svg)
			...
			.call(zoom().transform, transform.translate(0, 0).scale(4))
	);
		...
		<svg bind:this={svg} {width} {height} viewBox="{-width / 2} {-height / 2} {width} {height}">
			<g transform="translate({transform.x} {transform.y}) scale({transform.k} {transform.k})">
				...
				{#each nodeData as { x: cx, y: cy, type, activation }}
					...
					<text class="cursor-grab" {...nodeTypeProps}>{type}</text>
					{#if activation}
						<text class="cursor-grab" {...actProps}>{activation}</text>
					{/if}
				{/each}
			</g>
		</svg>
