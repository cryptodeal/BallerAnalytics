import { Player2, Team2, League, Official2, initConnect, endConnect, Game2 } from '../../../index';

const clearGameRef = async () => {
	for (const game of await Game2.find({ 'meta.helpers.bballRef.year': 2022 })) {
		await game.remove();
	}

	for (const player of await Player2.find()) {
		const i = player.seasons.findIndex((s) => s.year === 2022);
	...
	for (const team of await Team2.find()) {
		const i = team.seasons.findIndex((s) => s.season === 2022);
	...
	for (const official of await Official2.find()) {
		const i = official.seasons.findIndex((s) => s.year === 2022);
