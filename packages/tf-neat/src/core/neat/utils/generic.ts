 ...
 *   (in place).
 */
function convertNDArrayScalarsInConfig(config: serialization.ConfigDictValue): void {
...
export function debounce<T>(
	f: (...args: Array<object>) => T,
	waitMs: number,
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	nowFunc?: (...args: Array<any>) => number
): (...args: Array<object>) => T {
	let lastTime = nowFunc != null ? nowFunc() : util.now();
	let lastResult: T;
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const f2 = (...args: Array<any>) => {
 ...
 */
export function mapActivationToFusedKernel(activationName: string): fused.Activation | null {
