...
import {
	...
	tidy,
	where,
	zerosLike as coreZerosLike
} from '@tensorflow/tfjs-core';
import { checkDataFormat } from './common';
import { NotImplementedError, ValueError } from './errors';
import { DataFormat, Shape } from './keras_format/common';
import type { HasShape } from './types';
...
export function countParams(x: HasShape): number {
	const shape = x.shape;
	if (shape.length > 0) {
		return shape.reduce((a, b) => (a as number) * (b as number)) as number;
	} else {
		// Scalar.
		return 1;
	}
...
export function sliceAlongFirstAxis(array: Tensor, start: number, size: number): Tensor {
	return tidy(() => {
		switch (array.rank) {
			...
			case 2:
				return tfc.slice2d(array as Tensor2D, [start, 0], [size, array.shape[1] as number]);
			case 3:
				return tfc.slice3d(
					array as Tensor3D,
					[start, 0, 0],
					[size, array.shape[1] as number, array.shape[2] as number]
				);
			case 4:
				return tfc.slice4d(
					array as Tensor4D,
					[start, 0, 0, 0],
					[size, array.shape[1] as number, array.shape[2] as number, array.shape[3] as number]
				);
			case 5:
				return tfc.slice(
					array as Tensor5D,
					...
					[
						size,
						array.shape[1] as number,
						array.shape[2] as number,
						array.shape[3] as number,
						array.shape[4] as number
					]
				);
			case 6:
				return tfc.slice(
		...
		switch (array.rank) {
			...
			case 3:
				return tfc.slice3d(
					array as Tensor3D,
					[0, 0, start],
					[array.shape[0], array.shape[1] as number, size]
				);
			case 4:
				return tfc.slice4d(
					array as Tensor4D,
					[0, 0, 0, start],
					[array.shape[0], array.shape[1] as number, array.shape[2] as number, size]
				);
			default:
				throw new ValueError(
				...
				switch (axis) {
					...
					case 2:
						return tfc.slice3d(
							array as Tensor3D,
							[0, start, 0],
							[array.shape[0], size, array.shape[2] as number]
						);
					case 3:
						return sliceAlongLastAxis(array, start, size);
					default:
				...
				switch (axis) {
					...
					case 2:
						return tfc.slice4d(
							array as Tensor4D,
							[0, start, 0, 0],
							[array.shape[0], size, array.shape[2] as number, array.shape[3] as number]
						);
					case 3:
						return tfc.slice4d(
							array as Tensor4D,
							[0, 0, start, 0],
							[array.shape[0], array.shape[1] as number, size, array.shape[3] as number]
						);
					case 4:
						return sliceAlongLastAxis(array, start, size);
					default:
...
export function randomNormal(
	...
	return tfc.randomNormal(shape as number[], mean, stddev, dtype, seed);
}
...
export function dot(
	a: Tensor,
	b: Tensor,
	activation?: tfc.fused.Activation,
	bias?: Tensor
): Tensor {
	...
	if (a.rank === 2 && b.rank === 2) {
		...
		return tfc.fused.matMul({
			...
			transposeA,
			transposeB,
			bias: bias ? reshapeBias(a.rank, bias, imageDataFormat()) : undefined,
			activation
		});
	} else {
		...
		a = tfc.reshape(a, [-1, aLastDim as number]);
		...
		b = tfc.reshape(tfc.transpose(b, perm), [ySecondLastDim as number, -1]);
...
export function sign(x: Tensor): Tensor {
	...
	return tidy(() => {
		...
		return where(
			tfc.equal(x, zerosLikeX),
			zerosLikeX,
			where(tfc.greater(x, coreZerosLike(x)), onesLikeX, tfc.mul(-1, onesLikeX))
		);
	});
}
	...
	if (xRank === 5) {
		if (dataFormat === 'channelsFirst') {
			if (biasShape.length === 1) {
				...
				return tfc.reshape(bias, [
					1,
					biasShape[3] as number,
					biasShape[0] as number,
					biasShape[1] as number,
					biasShape[2] as number
				]);
			}
		...
		if (dataFormat === 'channelsFirst') {
			if (biasShape.length === 1) {
				...
				return tfc.reshape(bias, [1, biasShape[1] as number, biasShape[0] as number]);
			}
		} else if (dataFormat === 'channelsLast') {
			if (biasShape.length === 1) {
...
export function hardSigmoid(x: Tensor): Tensor {
	return tidy(() => {
		const y = tfc.add(0.5, tfc.mul(0.2, x));
