...
} from './keras_format/common';
import { checkStringTypeUnionValue } from './generic';
...
export function getUniqueTensorName(scopedName: string): string {
	...
	const index = nameMap.get(scopedName) as number;
	nameMap.set(scopedName, (nameMap.get(scopedName) as number) + 1);
